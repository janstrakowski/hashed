...
- **The root context** — active at the very start of a program, before any `withctx` — starts with `{ .permissions = { .io = nothing }, .cache = <the cache, below>, .dirs = <the directories, below> }`: I/O is allowed by default. `withctx` is how you *narrow* permissions around a sub-computation (e.g. before calling `import`ed code, §13), not how you grant them from nothing. Since `withctx` replaces the context wholesale (§7), a program that narrows permissions via a hand-built `Table` rather than `ctx concat {...}` loses `.cache` and `.dirs` too unless it explicitly carries them over.
- **`ctx.dirs` (added 2026-09-01)** is a `Table` of the directories the runtime opened for this program, by name — one per `--dir <name>=<path>` on the command line — and it is the only way a program ever reaches the filesystem. §16's operations all take a directory handle plus a sub-path, `.dir` is required on every one of them, and every handle a program holds traces back to an entry here or to a `loadfile` through one. **A run that names no directory hands over none, and such a program cannot touch the filesystem at all** — not because a permission stops it, but because it has nothing to name. Entries are ordinary directory `File` values (§3), indistinguishable from one obtained by `loadfile`.

  **A program's inputs are therefore explicit, and they are explicit in the run rather than in the source.** There is no working directory, no current directory to be relative to, and no path that resolves against the process instead of against a handle — the language has no such notion, and nothing is inferred from where a source file happens to sit. What a program may read and write is exactly what the command line said, which is why an example in this repository carries its own `run:` line: the flags are part of the example, not incidental to running it.

  Dropping a handle from `.dirs` for a sub-expression is as meaningful as dropping a permission, and reads the same: `<expr> withctx (ctx concat { .dirs = empty })` leaves `<expr>` able to use only handles it already holds. There is no way to *widen* the set from inside a program, since a handle can only ever be obtained from a handle.

  **They hash like any other directory (resolved 2026-09-01), which §15 can feel.** An entry of `.dirs` is a directory `File`, so §3's rule applies unchanged: it hashes over its contents. `cached` folds `ctx` into a key whenever the cached expression mentions `ctx` (§15), so such an entry is keyed on the *contents of the directories the program was handed* — a hit while its inputs are as they were, a miss once any of them changes. That is coarse, deliberately: the alternative was to hash these unlike every other directory, and a declared input changing is exactly the thing a build cache should notice. It does mean a handle should name what a program actually reads rather than a tree it happens to sit in, since everything under it is then part of the key.
...
**What a key reaches (added 2026-09-01).** A cached expression is hashed as a closure, so a key covers the values it captures — and, when the expression mentions `ctx`, the context it runs under. Since `ctx.dirs` (§9) holds directory `File`s and a directory hashes over its contents (§3), a cached expression that reads through a handle is keyed on the contents of the directories the run handed the program. It hits while those are unchanged and misses once any of them changes, which is the coarse but honest reading of "the declared inputs changed". Handing over a directory much larger than what is actually read makes keys change more often than they need to.
...
The four filesystem operations below are gated by [`ctx.permissions.io`](#9-implicit-names) (present ⇒ allowed) at the moment they're actually called — see "Context & permissions" (§9) for why builtins check `ctx` live rather than capturing it. Denied or failed calls raise into the ordinary failure channel (§8) — **fatal**, exactly like a failed `check`/`error` (§11), and not caught by any enclosing `else`. A missing file, a malformed sub-path, a denied `io` permission: each of these ends the evaluation rather than yielding a recoverable value (see §8's failure semantics, which this sentence used to contradict by calling them catchable).

**Every operation is (directory handle, sub-path) — revised 2026-09-01.** `.dir` is required on every one of them, and there is no form that takes a path on its own — a bare `loadfile "notes.txt"` is an error, not a shorthand, since there is nothing for it to resolve against. A program reaches the filesystem only through handles: the ones the runtime gave it in `ctx.dirs` (§9), and the ones it opened from those.

**What a sub-path may be.** A non-empty sequence of ordinary names, separated by `/`. Specifically it may **not**:

- start at a root — `/x`, and on Windows also `C:/x`, `C:x`, `//server/share/x`, and `name:stream` (an alternate data stream is a second body behind one entry, not a name inside a directory);
- contain a `.` or `..` segment **anywhere**, including where they would cancel out: `.`, `..`, `a/./b` and `a/../b` are all refused, and so is a bare `.` naming the handle itself — a handle is already a value, so there is nothing to name;
- contain an empty segment, which makes `a//b` and a trailing `a/` refusals too.

The `.`/`..` rule is deliberately blunt. Refusing `a/./b` costs a spelling nobody needs and buys a rule that fits in one sentence: a reader never has to reduce a path in their head to see which directory a call lands in, and the cases that *would* reduce to somewhere outside stop being a special case to get right, since none of the three spellings exists at all.

**What containment then guarantees.** A sub-path resolves inside its handle's directory, and cannot leave it — not via `..` (which no longer parses as a path), not via a root, and not through a **symlink**: every component, the last one included, is opened no-follow, so a symlink anywhere along the way is rejected outright rather than followed, whether or not it would have resolved back inside. Hand a program one directory handle and there is no expressible way for it to read or write outside that tree. **Implementation note (revised 2026-08-26):** originally specified as enforced via Linux's `openat2`/`RESOLVE_BENEATH`; in practice that hit an environment-specific kernel bug during development (`openat` via a real directory fd failing unless `O_CREAT` is set, on at least one WSL2 build), so the walk is done manually with the long-portable `*at()` syscall family instead of the newer, less consistently supported `openat2`.

- **`loadfile { .dir = <handle>, .path = <sub_path> }`** — reads the file or directory at `<sub_path>` (a `Utf8`) inside `<handle>` (a directory `File`, from `ctx.dirs` or from an earlier `loadfile`), returning it as a `File` (§3).
- **A directory `File`, however obtained, can itself be used as `<handle>` in a further `loadfile`/`createfile`/`symlink`/`readlink` call** — this is how a tree gets traversed: open a directory once, then address everything inside it relative to that one handle, each access independently contained to it.
- **`createfile { .dir = <handle>, .path = <sub_path>, .content = <value> }`** — creates a new file at `<sub_path>` inside `<handle>` with `<content>` (`Utf8` or `Bytes`) as its bytes, returning the `File` it just created. **Exclusive for now**: fails if a file already exists at that path — no overwrite mode yet. (`.dir = ctx.cache` is the one case that takes no `.path`; see below.)
- **`symlink { .dir = <handle>, .path = <sub_path>, .target = <value> }`** — creates a symlink at `<sub_path>` whose target string is `<value>` (`Utf8`). The target is stored as-is, not validated or resolved — consistent with §3's directory-hash treatment of symlinks ("target is NOT followed/resolved"). Note that the target is a *string*, not a sub-path, and is not subject to the rules above; what it is subject to is that nothing in this section will ever follow it. Returns `nothing` — a symlink isn't a `File` value in its own right (§3).
- **`readlink { .dir = <handle>, .path = <sub_path> }`** — reads the target string of the symlink at `<sub_path>` without following it, returning it as `Utf8`.
