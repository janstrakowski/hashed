# The Hashed language, feature by feature
...
```sh
./hl -e '{ .a = 1 } concat { .b = 2 }'      # one expression, then exit
./hl examples/tables-map.hl                 # a whole file
./hl                                        # a REPL: expression, blank line
```
...
program's output, which `hl` prints.
...
→ `examples/arithmetic.hl`, `examples/strings.hl`,
`examples/nothing-and-empty.hl` (§3, §4, §6)
...
→ `examples/numeric-literals.hl` (§3)
...
→ `examples/arithmetic.hl`, `examples/comparison-and-logic.hl` (§4, §6, §8)
...
→ `examples/tables-map.hl`, `examples/tables-sequence.hl`,
`examples/table-and-concat.hl` (§5)
...
→ `examples/functions.hl`, `examples/functions-and-holes.hl` (§7)
...
→ `examples/recursion.hl`, `examples/recursion-anonymous.hl` (§9/§10)
...
→ `examples/guard-chain.hl`, `examples/table-destructuring.hl`,
`examples/sequence-pattern.hl` (§8)
...
→ `examples/variant.hl`, `examples/variants-dynamic.hl`,
`examples/optional.hl`, `examples/nothing-and-empty.hl` (§5, §8)
...
→ `examples/check-and-invariants.hl` (§11)
...
→ `examples/files-sandboxed.hl`, `examples/files-symlink.hl`,
`examples/option-picker.hl` (§3, §16)
...
→ `examples/hashing-directories.hl` (§3, §9)
...
→ `examples/hashing-functions.hl` (§15, §16)
...
→ `examples/hashing-cyclic.hl` (§6, §10, §15)
...
`$XDG_CACHE_HOME/hashed`, else the per-user default — one entry per key,
named `sha256-<key>`:

```
<cache>/sha256-<key>                  a File value, stored as itself
<cache>/sha256-<key>.hl/              anything else
<cache>/sha256-<key>.hl/value.hl        the value, as Hashed text
<cache>/sha256-<key>.hl/sha256-<h>      each File inside it, by content hash
```
...
→ `examples/context-permissions.hl`, `examples/option-picker.hl` (§9, §16)
...
→ `examples/async-basics.hl`, `examples/async-table.hl`,
`examples/async-branching.hl` (§2)
...
```
odin build src -out:hl.exe
.\hl.exe examples/option-picker.hl
```

Everything in this document works there — the same parser, evaluator, `async`
on real threads, the filesystem builtins, and `hl.exe -i` for the live editor
...
- **`ctx.cache` lives in `%LOCALAPPDATA%\hashed`** rather than
  `~/.cache/hashed`. `XDG_CACHE_HOME` still wins where it is set, and
...
`examples/files-symlink.hl` has nothing to read — the test suite detects that
...
```sh
scripts/build_wasi.sh                      # portable -> hl.wasm
wasmtime run --dir=. hl.wasm examples/tables-map.hl

scripts/build_wasi.sh --threads            # wasi-threads -> hl-threads.wasm
iwasm --max-threads=8 --dir=. hl-threads.wasm examples/async-table.hl
```
...
[**The terminal**](https://janstrakowski.github.io/hashed/playground.html)
...
$ cat examples/guard-chain.hl
$ hl examples/guard-chain.hl  run it
$ hl -e '1 + 2 * 3'           evaluate an expression
$ hl                          the REPL, reading what you type
```

`hl` there is the interpreter's **real** REPL loop, blocking on stdin: the
banner, the `hl> ` and `... ` prompts and `:q` come out of the module, not out
...
`hl -i` opens the **live editor** in there too: the same two-to-four-pane TUI as
...
in the filesystem, `ctx.cache` writes to `/cache/hashed`, and both survive
